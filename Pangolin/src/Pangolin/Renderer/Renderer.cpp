...
namespace Pangolin {
	...
	{
		shader->bind();
		std::dynamic_pointer_cast<OpenGLShader>(shader)->opengl_set_uniform_mat4("u_view_projection", _scene_data->view_projection_matrix);
		std::dynamic_pointer_cast<OpenGLShader>(shader)->opengl_set_uniform_mat4("u_transform", transform);
